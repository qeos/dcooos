...
u8 *get_free_page_notmarked(u8 *pdir, u8 init_addr){
#if DEBUG(E_NOTICE, ES_PAGING)
    printk_syslog_timestamp();
    printk_syslog("PAGING: get_free_page_notmarked(0x");
    printk_syslog_number(pdir, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(init_addr, 'h');
    printk_syslog(")\n");
#endif // DEBUG
    ...
    while((memorymap[index].state != PS_FREE) && (memorymap[index].state != PS_NULL)){
        ...
        if (index > maxmem /PAGE_SIZE){
            printk_syslog_timestamp();
            printk_syslog("PAGING: No free mem pages. We need caching.\n");
            show_map();
...
#if DEBUG(E_NOTICE, ES_PAGING)
    printk_syslog_timestamp();
    printk_syslog("PAGING: get_free_page(0x");
    printk_syslog_number(pdir, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(init_addr, 'h');
    printk_syslog(")\n");
...
#if DEBUG(E_NOTICE, ES_PAGING)
    printk_syslog_timestamp();
    printk_syslog("PAGING: paging_callback(0x");
    printk_syslog_number(reg->rip, 'h');
    printk_syslog(")\n");
#endif // DEBUG

    if(memorymap[cr2/PAGE_SIZE].state == PS_VIDEO){
        make_physical(cr3, cr2, cr2, PS_VIDEO);
        //paging_print_pdir(current_task->pdir);
    }else if((cr2 < maxmem) && (memorymap[cr2/PAGE_SIZE].state == PS_FREE)){
...
#if DEBUG(E_NOTICE, ES_PAGING)
    printk_syslog_timestamp();
    printk_syslog("PAGING: make_physical(0x");
    printk_syslog_number(pdir, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(log_addr, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(phys_addr, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(need_markup, 'h');
    printk_syslog(")\n");
#endif // DEBUG
...
void make_physical_sized(u8 *pdir, u8 log_addr, u8 phys_addr, u8 size_addr, u1 need_markup){
#if DEBUG(E_NOTICE, ES_PAGING)
    printk_syslog_timestamp();
    printk_syslog("PAGING: make_physical_sized(0x");
    printk_syslog_number(pdir, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(log_addr, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(phys_addr, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(size_addr, 'h');
    printk_syslog(", 0x");
    printk_syslog_number(need_markup, 'h');
    printk_syslog(")\n");
#endif // DEBUG
